...
class StackResourceRegistryFactoryBeanTest {
	...
	private static AmazonCloudFormation makeAmazonCloudFormationClient(Map<String, String> resourceIdMappings) {
		...
		for (Map.Entry<String, List<StackResourceSummary>> entry : stackResourceSummaries.entrySet()) {
			...
			when(amazonCloudFormationClient
					.listStackResources(argThat(item -> item != null && stackName.equals((item).getStackName()))))
	...
	void createInstance_stackWithNextTag_returnsStackResourceRegistryBuildWithTwoPages() throws Exception {
		...
		when(cloudFormationClient.listStackResources(new ListStackResourcesRequest().withStackName(STACK_NAME)))
		...
		when(cloudFormationClient
				.listStackResources(new ListStackResourcesRequest().withStackName(STACK_NAME).withNextToken("2")))
