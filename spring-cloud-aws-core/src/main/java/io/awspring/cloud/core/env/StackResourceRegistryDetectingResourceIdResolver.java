 ...
 * for stack resources exposed via {@link StackResourceRegistry} instances. This
 * implementation automatically detects the single, optional stack resource
 * registry bean and uses it for resolving logical resource ids to physical
 * ones. If no stack resource registry bean can be found in the bean factory, a
 * pass-through stack resource registry is used instead, which always returns
 * the provided logical resource id as the physical one.
 *
 * @author Christian Stettler
 */
// TODO discuss whether to support more than one stack resource registry and how
// to deal
...
public class StackResourceRegistryDetectingResourceIdResolver
	...
	/**
	 * Resolves the provided logical resource id to the corresponding physical
	 * resource id. If the logical resource id refers to a resource part of any of
	 * the configured stacks, the corresponding physical resource id from the stack
	 * is returned. If none of the configured stacks contain a resource with the
	 * provided logical resource id, or no stacks are configured at all, the logical
