...
public interface DatabasePlatformSupport {
	...
	/**
	 * Returns the fully qualified driver class name for the database platform. Implementation
	 * may return one static driver class name or check for the existence of different driver
	 * class on the class path and return the best matching one.
	 * @param databaseType - The database type for which the database driver class should be
	 *     returned.
	...
	/**
	 * Constructs the database URL for the database instance. Implementation will typically
	 * infer the URI scheme from the database type (e.g. for mysql jdbc:mysql). The remaining
	 * parts like the hostname and port are used to fully construct the url based on the
	 * database platform.
