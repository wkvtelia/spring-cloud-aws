 ...
 * This annotation is a meta-annotation of
 * {@link org.springframework.cache.annotation.EnableCaching} with the same
 * configuration attributes. In contrast to the
 * {@link org.springframework.cache.annotation.EnableCaching} annotation this
 * annotation does not require a
 * {@link org.springframework.cache.annotation.CachingConfigurer} instance,
 * because it is provided by this annotation itself. Therefore users only need
 * to add this annotation into one configuration file and configure the
 * necessary cache names.
 *
 * If there is no
 * {@link io.awspring.cloud.cache.config.annotation.EnableElastiCache#value()}
 * ()} attribute configured, then all caches inside a stack (if the application
 * is running in one stack) will be used.
 *
 * @author Agim Emruli
...
public @interface EnableElastiCache {
	 ...
	 * multiple caches
	 * {@link io.awspring.cloud.cache.config.annotation.CacheClusterConfig}
	 * configurations with their physical cache name (as configured in the
	 * ElastiCache service) or their logical cache name if the caches are configured
	 * inside a stack and
	 * {@link io.awspring.cloud.context.config.annotation.EnableStackConfiguration}
	 * annotation is used inside the application.
	 *
	 * The CacheClusterConfig annotation also configures cache specific attributes
	 ...
	 * expiration time configuration with a
	 * {@link io.awspring.cloud.cache.config.annotation.CacheClusterConfig}
	 * configuration for the cache. The expiration time is implementation specific
	 * (e.g. Redis or Memcached) and could therefore differ in the behaviour based
	 * on the cache implementation.
	 * @return - the default expiration time for all caches that do not contain a
	 * specific expiration time on cache level
	 */
